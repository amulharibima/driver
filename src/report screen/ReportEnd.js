...
import {REPORT_IMAGE, REPORT, FINISH} from '../redux/case type/DriverCase';
import {Modal} from 'react-native-paper';
import {reportEnd} from '../redux/action/DriverAction';
...
const ReportEnd = () => {
  ...
  const sendPict = async () => {
    ...
    if (picture.length > 0 || Object.values(picture).length > 0) {
      ...
      try {
        const url = EndPoint.FINISH_ORDER + order_id;
        const token = await AsyncStorage.getItem('userToken');
        const res = await Axios.post(
          // `${API_ORDER}finish/${order_id}`,
          url,
          formData,
          {
            headers: {
        ...
        {send === false ? (
          ...
          <View style={styles.sendContainer}>
            ...
            <Text style={styles.message}>
              {lng === true
                ? `The car pictures has been sent. Please tap "Back to Home" to start looking for other trips`
                : `Foto mobilnya sudah terkirim. Silahkan ketuk “Kembali ke Menu
              Awal” untuk mulai mencari perjalanan lainnya`}
            </Text>
            <Button
              title={lng === true ? 'Back to Home' : 'Kembali ke Menu Awal'}
